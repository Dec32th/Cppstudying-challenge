...
//만약에 help silde의 1번(page 7)의 형태로 코드를 작성하게 되면, 왼쪽의 subtree의 가장 큰 값이 root의 값보다 더 크게 될 수 있고, 오른쪽 subtree의 값 중 가장 작은 값이 root의 값보다 작아질 수도 있다는 문제를 가지고 있다.
//그 경우에는 binary search tree가 아닌데도 binary search tree로 판단하는 오류가 발생한다.
bool TreeType::Imp_IsBST(TreeNode* tree, ItemType& min, ItemType& max)
{
    bool isBST;
    ...
    min = (tree->left == NULL) ? tree->info : left_min;
    min = (tree->right == NULL) ? tree->info : right_max;
...
TreeNode* TreeType::GetRoot() const
{
    return root;
}
