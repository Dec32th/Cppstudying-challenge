...
bool GraphType<VertexType>::DepthFirstSearch(VertexType startVertex, VertexType endVertex)
{
	...
	GetToVertices(startVertex, vertexQ); // startVertex의 인접 정점들을 큐에 집어넣음
	...
	while (!vertexQ.IsEmpty())
	{
		...
		if (!IsMarked(vertex))
		{
			MarkVertex(vertex); // 정점을 방문했음을 표시

			// vertex를 시작 노드로 하여 DepthFirstSearch를 재귀 호출
