...
// root 노드를 제외하고 나머지 노드들은 heap의 조건을 만족하고 있음
{
    ...
    while (leftChild <= bottom && !reheaped)
    {
        if (leftChild == bottom) // 왼쪽 자식 노드 하나만 있는 경우
            ...
            rightChild = leftChild + 1; // 오른쪽 자식노드의 위치 계산
            maxChild = (rightChild <= bottom && elements[leftChild] < elements[rightChild]) ? rightChild : leftChild;
        }

        if (elements[root] < elements[maxChild]) {
            Swap(elements[root], elements[maxChild]);
            root = maxChild; // maxChild 가 root의 새로운 위치가 됨
            leftChild = (2 * root) + 1; // root 값으로부터 새로운 왼쪽 자식노드의 위치 계산
        }
        else
            reheaped = true;
    }
